...
public class LevelGeneratorSecondSolution
{
    ...
    {
        LevelGeneratorSecondSolution levelGenerator = new LevelGeneratorSecondSolution(width, height);
    ...
    private LevelGeneratorSecondSolution(int width, int height)
    {
        this.width = width;
        this.height = height;
    }
    ...
    private Level createLevel(long seed, int difficulty, int type)
    {
        ...
        this.type = type;
        this.difficulty = GeneticLevelGeneratorSecondSolution.initialDifficulty;
...
//        Individual generatedLevel = geneticLevel.createLevelGen(seed);
        ...
        // Nivel genético.
        GeneticLevelGeneratorSecondSolution geneticLevel = new GeneticLevelGeneratorSecondSolution(height, width);

        IndividualSecondSolution generatedLevel = geneticLevel.createLevelGenSecondSolution(seed);

        // Generación de elementos del nivel.
        for (LevelElementSecondSolution element : generatedLevel.getIndividual()) {
   ...
   // Se dibujan en el mapa partes del nivel. Cada m�todo devuelve la longitud de el bloque elegido.
    ...
    private int buildCannons(int xo, int length, int elementHeight, int maxLength)
    {
        ...
        for (int x = xo; x < xo + length; x++)
        {
            ...
            if (x > xCannon && GeneticLevelGeneratorSecondSolution.initialDifficulty > 2)
            {
                xCannon += 2 + random.nextInt(4);
            }
            if (xCannon == xo + length - 1 && GeneticLevelGeneratorSecondSolution.initialDifficulty > 2) xCannon += 10;
